...
export default async function RankPage() {
  ...
  const canManageRank =
    !!currentUser &&
    ((await hasAnyLoreCapability(currentUser.id, "ADMIN")) ||
  ...
  const [boards, favorites] = await Promise.all([
    ...
    currentUser
      ? prisma.favorite.findMany({
          where: { userId: currentUser.id, rankItemId: { not: null } },
          select: { rankItemId: true },
        })
      : Promise.resolve([]),
  ]);
  ...
  const favoriteIds = new Set(
    favorites.map((item) => item.rankItemId).filter(Boolean)
  );
                  ...
                  <Link href={`/rank/${item.id}`} className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-4">
                      <div className="min-w-0 space-y-1">
                        ...
                        <p className="text-xs text-[#8f968d]">
                          {item._count.votes} {"\u6b21\u6295\u7968 \u00b7"} {item._count.comments} {"\u6761\u8bc4\u8bba"}
                        </p>
                      </div>
