...
export default async function MePage() {
    ...
    return (
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-10 sm:px-6">
        <div className="rounded-3xl border border-zinc-800 bg-zinc-950/80 p-8 text-center">
          ...
          <p className="mt-3 text-sm text-zinc-400">请先登录后查看你的帖子、评论、投票和收藏。</p>
  ...
  const [posts, comments, votes, favorites] = await Promise.all([
    ...
    prisma.favorite.findMany({ where: { userId: currentUser.id }, include: { loreEntry: true, post: true, rankItem: { include: { board: true, loreEntry: true } } }, orderBy: { createdAt: "desc" } }),
  ]);
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10 sm:px-6">
      <section className="rounded-3xl border border-zinc-800 bg-zinc-950/80 p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-400">查看你最近发布的内容、互动记录和收藏内容。</p>
          </div>
          <div className="flex gap-3">
            ...
            <Link href="/me/favorites" className="rounded-full border border-zinc-700 px-4 py-2 text-sm text-zinc-100 transition hover:border-zinc-500 hover:bg-zinc-900">我的收藏</Link>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-5"><p className="text-sm text-zinc-400">我的收藏</p><p className="mt-3 text-3xl font-semibold text-zinc-100">{favorites.length}</p></div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        <div className="rounded-3xl border border-zinc-800 bg-zinc-950/70 p-6">
          ...
          <div className="mt-4 space-y-3">{votes.length === 0 ? <p className="rounded-2xl border border-dashed border-zinc-800 px-4 py-6 text-sm text-zinc-400">你还没有参与战力榜投票。</p> : votes.map((vote) => <Link key={vote.id} href={`/rank/${vote.rankItemId}`} className="block rounded-2xl border border-zinc-800 px-4 py-4 transition hover:border-zinc-600 hover:bg-zinc-900/60"><p className="text-sm font-medium text-zinc-100">{vote.rankItem.loreEntry?.title ?? "未绑定词条"}</p><p className="mt-2 text-sm text-zinc-400">{vote.rankItem.board.title} · 当前分数 {vote.rankItem.score}</p><p className="mt-2 text-xs text-zinc-500">你的投票倾向：{vote.value > 0 ? "支持当前" : vote.value === 0 ? "保持不变" : "应更低"}</p></Link>)}</div>
        </div>

        <div className="rounded-3xl border border-zinc-800 bg-zinc-950/70 p-6">
          <div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-zinc-100">我的收藏</h2><Link href="/me/favorites" className="text-sm text-zinc-400 transition hover:text-zinc-100">查看全部</Link></div>
          <div className="mt-4 space-y-3">{favorites.length === 0 ? <p className="rounded-2xl border border-dashed border-zinc-800 px-4 py-6 text-sm text-zinc-400">你还没有收藏任何内容。</p> : favorites.map((favorite) => { const href: Route = favorite.loreEntry ? `/world/${favorite.loreEntry.slug}` as Route : favorite.post ? `/community/${favorite.post.id}` as Route : favorite.rankItem ? `/rank/${favorite.rankItem.id}` as Route : "/me/favorites"; const label = favorite.loreEntry ? "词条" : favorite.post ? "帖子" : "排行"; const title = favorite.loreEntry?.title ?? favorite.post?.title ?? favorite.rankItem?.loreEntry?.title ?? "未命名内容"; return <Link key={favorite.id} href={href} className="block rounded-2xl border border-zinc-800 px-4 py-4 transition hover:border-zinc-600 hover:bg-zinc-900/60"><p className="text-xs text-zinc-500">{label}</p><p className="mt-1 text-sm font-medium text-zinc-100">{title}</p><p className="mt-2 text-xs text-zinc-500">{formatDate(favorite.createdAt)}</p></Link>; })}</div>
        </div>
      </section>
    </main>
  );
